# -----------------------------------------------------------------------------------
# Efficient Dual Attention Transformer (EDAT)
# -----------------------------------------------------------------------------------
import math
import torch
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0.):
        ...
        self.biases = torch.stack(torch.meshgrid([position_bias_h, position_bias_w]))  # 2, 2*Wh-1 * 2*Ww-1
...
class GlobalAttentionBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        coords_q = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, ws, ws
        coords_q_flatten = torch.flatten(coords_q, 1)  # 2, ws*ws
        
        coords_h = torch.arange(size_for_kv_h)
        coords_w = torch.arange(size_for_kv_w)
        coords_kv = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, wse, wse
